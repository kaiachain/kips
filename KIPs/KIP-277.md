...
| **Node ID**           | The node address assigned for each staking contract registered in the AddressBook. Please note that one validator can have multiple node IDs but only one consensus node ID. |
| **Consensus Node ID** | The node address participating in the consensus. It uniquely identifies a validator.                                                                                         |
...
1. Candidate manager deploys a new consensus staking contract (`CnStakingV3MultiSig`) through the registered staking factory.
2. Candidate finishes the initialization of staking contract.
3. Candidate submits an onboarding request to `VMC`.
...
1. Validator manager submits an offboarding request to `VMC`.
...
1. Validator maanger deploys a new staking contract through the registered staking factory.
...
1. Validator manager submits a remove staking contract request to `VMC`.
2. Kaia team reviews the request.
3. Kaia team approves the request.
...
The `VMC` contract will be backward compatible with the existing `CnStakingContractV2` and `CnStakingContractV3`, but not compatible with the previous deprecated `CnStakingContractV1`.
