...
This KIP introduces a transaction bundling mechanism; it enables all-or-nothing atomicity on multiple transactions, provides timeout guard, and facilitates a proposer to insert transactions signed by it.
...
```
// work.commitBundleTransaction(tx, bundle, ...)
if tx failed:
  RevertToSnapshot(preBundleSnapshot)

if tx succeeded:
  if bundle is nil:
    append to env.txs and env.receipts
  else if tx is the last in bundle:
    append all txs and receipts in bundle to env.txs and env.receipts
```

Also, all the transactions in the bundle must be removed:

```
// work.ApplyTransactions(txs, ...)
err, logs := env.commitBundleTransaction(tx, bundle, bc, rewardbase, vmConfig)
if tx failed:
  pop from txs all transactions in bundle
```
