...
State transitions occur either **at epoch intervals** or **anytime**. All the events occurred at block `N` that requires state transition must be reflected since the block `N+1` and state transition must ensure block-level atomicity. For example, if a validator in `ValActive` requests to be `ValPaused` at block `N`, it will be `ValPaused` since the block `N+1` and any state access at block `N` must return `ValActive`.
...
| From | To | Timing | Condition |
|------|-----|--------|-----------|
| ValActive | ValPaused | Anytime | `paused slot count` < `ValPausedSlotLimit` AND request by self for maintenance |
| ValActive | ValPaused | Anytime | `paused slot count` < `ValPausedSlotLimit` AND caught by minor VRank violation |
| ValPaused | ValActive | Anytime | - |
| ValPaused | ValInactive | Anytime | `paused duration` >= `ValPausedTimeout` |
| ValPaused | ValInactive | Epoch | Below Top 50 by stake |

#### Exit & Offboarding

| From | To | Timing | Condition |
|------|-----|--------|-----------|
| ValActive | ValExiting | Anytime | `exiting slot count` < `ValExitingSlotLimit` AND request by self for offboarding |
| ValActive | ValExiting | Anytime | `exiting slot count` < `ValExitingSlotLimit` AND caught by severe VRank violation |
| ValExiting | ValInactive | Epoch | Automatic at next epoch |
| ValInactive | CandInactive | Anytime | `inactive duration` >= `ValInactiveTimeout` |
| ValInactive | CandInactive | Anytime | Request by self for offboarding |
    ...
    for entity in eligible_pool:
        if entity not in top50:
            if entity.state == ValActive:
                ...
                transition(entity, ValInactive)
            # ValReady stays ValReady (waiting for top 50 slot)
...
flowchart LR
    ...
    ValInactive -->|"timeout"| CandInactive
