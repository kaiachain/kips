...
| From | To | Timing | Condition |
|------|-----|--------|-----------|
| ValActive | ValPaused | Anytime | ValPaused slot is available AND request by self for maintenance |
| ValActive | ValPaused | Anytime | ValPaused slot is available AND caught by minor VRank violation |
| ValPaused | ValActive | Anytime | - |
| ValPaused | ValInactive | Anytime | Timeout |
...
| From | To | Timing | Condition |
|------|-----|--------|-----------|
| ValActive | ValExiting | Anytime | ValExiting slot is available AND request by self for offboarding |
| ValActive | ValExiting | Anytime | ValExiting slot is available AND caught by severe VRank violation |
| ValExiting | ValInactive | Epoch | Automatic at next epoch |
| ValInactive | CandInactive | Anytime | Timeout |
...
flowchart LR
    ...
    CandReady -.->|"start testing"| CandTesting
    CandTesting -.->|"failed VRank"| CandInactive
    CandTesting -.->|"pass & top 50"| ValActive
    CandTesting -.->|"pass & below top 50"| ValInactive

    %% Validator Active Set
    ValActive -.->|"below top 50"| ValInactive
    ValInactive -->|"signal ready"| ValReady
    ValReady -->|"cancel ready"| ValInactive
    ValReady -.->|"top 50"| ValActive
