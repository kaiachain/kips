...
| From | To | Timing | Condition | Trigger |
|------|-----|--------|-----------|---------|
| CandInactive | CandReady | Anytime | Over `MinStake` AND `ready candidate count` < `MaxReadyCandidateCount` AND `validator count + ready candidate count` < `MaxValidatorCount` | User tx |
...
| From | To | Timing | Condition | Trigger |
|------|-----|--------|-----------|---------|
| ValActive | ValPaused | Anytime | `paused slot count` < `ValPausedSlotLimit` AND request by self for maintenance | User tx |
| ValActive | ValPaused | Anytime | `paused slot count` < `ValPausedSlotLimit` AND caught by minor VRank violation | System tx |
...
| From | To | Timing | Condition | Trigger |
|------|-----|--------|-----------|---------|
| ValActive | ValExiting | Anytime | `exiting slot count` < `ValExitingSlotLimit` AND request by self for offboarding | User tx |
| ValActive | ValExiting | Anytime | `exiting slot count` < `ValExitingSlotLimit` AND caught by severe VRank violation | System tx |
| ValExiting | ValInactive | Epoch | - | System tx |
| ValInactive | CandInactive | Anytime | `inactive duration` >= `ValInactiveTimeout` | System tx |
