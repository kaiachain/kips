...
Epoch transitions occur at end of last epoch block (`(block.number + 1) % VRankEpoch == 0`) after processing all transactions. The following pseudo code defines the transition ordering:

```python
def process_epoch_transition():
    # T1: Clear transitional states
    # ValExiting validators have completed their exit period
    ...
    for entity in eligible_pool:
        if entity in top50:
            ...
            # ValPaused stays ValPaused (requires voluntary recovery via anytime transition)
