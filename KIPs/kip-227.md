...
2. `cfReport(N+1)` MUST include each candidate (in `CandTesting` at block `N`) who either (a) did not send a `VRankCandidate` for block `N` on-time, or (b) sent an invalid message (including ECDSA or BLS signature failure, or a missing KIP-113 BLS key registration). If block `N` reached consensus at a round greater than `MAX_ROUND`, `cfReport(N+1)` MUST instead be empty, since `VRankCandidate` messages above `MAX_ROUND` are not collected and candidate on-time status cannot be determined for such a view.
