...
- **BLS signature** (`BlsSig`): MUST be produced with the candidate's registered BLS key (per KIP-113) over `blockNumber` encoded as a 32-byte big-endian integer (same encoding as the Randao random-reveal message).
...
type VRankCandidate struct {
	...
	BlsSig      []byte // BLS signature over blockNumber (32-byte big-endian)
}
```
