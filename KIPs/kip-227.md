...
**Total Candidate Failure Score (TCFS)**: For epoch index `k`, TCFS MUST be computed from `cfReport(N)` for blocks `N ∈ [k*EPOCH_LENGTH, (k+1)*EPOCH_LENGTH - 1]` (note that `cfReport(k*EPOCH_LENGTH)` is empty).
