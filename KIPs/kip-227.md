...
| `MAX_ROUND`                 | Highest consensus round for which candidate evaluation is performed (range `[0, MAX_ROUND]`). Default = 10. `VRankCandidate` messages for rounds above `MAX_ROUND` are not collected.      |
...
> **Note**: The index `N` of `cfReport(N)` refers to the block in which the report is **recorded**, not the block it evaluates. A `cfReport` is written about the writer's own most recent prior proposal and carries that target block:
>
> - **Writer (proposer of block `N`)**: evaluates the `VRankCandidate` messages it collected for `T`, its own most recent prior proposal in the same epoch (so `GetProposer(T) == GetProposer(N)`), and writes `T` together with the failed candidates into `header(N).VRank`.
> - **Reader (any node)**: decodes `header(N).VRank` to obtain the target block `T` and its failed candidates.
>
...
**cfReport(N)** (Candidate Failure Report): Recorded in block `N`, it covers candidate evaluation during block `T`, the proposer's own most recent prior proposal in the same epoch (`GetProposer(T) == GetProposer(N)`). It carries `T` and the list of candidates (nodes in `CandTesting` at block `T`) that failed to send a valid `VRankCandidate` message on-time for block `T`.

Format: `cfReport(N) -> { target: T, candidates: [candidateAddr1, candidateAddr2, ...] }` with at most one entry per candidate of block `T`.
...
VRank runs in parallel with consensus. `pfReport(N)` is produced during block `N`'s consensus and committed in the next block header. A `cfReport` for a proposer's own block is produced from that block's consensus but committed later, in that proposer's next proposal, tagged with the evaluated (target) block.
...
2. Each validator MUST collect `VRankCandidate` messages from candidates in `CandTesting` and record each message's arrival time. A message is considered valid only if both its ECDSA signature (`Sig`) and its BLS signature (`BlsSig`) are valid. The BLS public key is resolved from the candidate's KIP-113 registration at the current chain head.
...
4. A candidate is counted as on-time if the message is valid and either (a) it arrives before `preprepared_time`, or (b) `arrival_time - preprepared_time ≤ CANDIDATE_MSG_TIMEOUT`. Otherwise it is a candidate failure for block `N`, which block `N`'s proposer records in the `cfReport` of its own next proposal (tagged with target block `N`).
...
#### Proposer writing a cfReport

When a proposer produces block `N`, it reports the candidate evaluation for its own most recent prior proposal, so that the report writer is the same validator that solicited the candidates (any withheld or fabricated failure is then attributed to that proposer's own column under CFS aggregation).

1. The proposer MUST set `header.VRank` per the encoding table in [Block Header Extension (VRank)](#block-header-extension-vrank).
2. Let `T` be the proposer's most recent prior block in the same epoch as `N` that it produced (`GetProposer(T) == GetProposer(N)`). If no such block exists — the proposer's previous proposal was in an earlier epoch, or it has not produced a block yet in this epoch — `header(N).VRank` MUST be empty.
3. `cfReport(N)` MUST target `T` and include each candidate (in `CandTesting` at block `T`) who either (a) did not send a `VRankCandidate` for block `T` on-time, or (b) sent an invalid message (including ECDSA or BLS signature failure, or a missing KIP-113 BLS key registration). If block `T` reached consensus at a round greater than `MAX_ROUND`, `cfReport(N)` MUST instead be empty, since `VRankCandidate` messages above `MAX_ROUND` are not collected and candidate on-time status cannot be determined for such a view.
...
- At a non-epoch block with a non-empty payload, the target block `T` MUST be a prior block in the same epoch (`T < N` and `epochStart(T) == epochStart(N)`) and MUST have the same proposer as `N` (`GetProposer(T) == GetProposer(N)`). The failed-candidate list MUST be sorted in ascending byte order, contain at most one entry per candidate ID, and each entry MUST be a candidate address from `CandTesting(T)` (equivalently `CandTesting(N-1)`, since `CandTesting` is stable within an epoch).
...
_Note: Each `cfReport(N)` is in the header of block `N` and reports on its target block `T` — the proposer's own most recent prior proposal (`GetProposer(T) == GetProposer(N)`)._
...
See the [permissionless branch](https://github.com/kaiachain/kaia/tree/permissionless) for the reference implementation.
