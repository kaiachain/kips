...
Format: `cfReport(N, R=0) -> [candidateAddr1, candidateAddr2, ...]` with at most one entry per candidate of previous block (`candidate(N-1)`).
...
**Proposal Failure Score (PFS)**: For epoch index `k`, PFS MUST be computed from `pfReport(N)` for blocks `N ∈ [k*EPOCH_LENGTH, (k+1)*EPOCH_LENGTH - 1]`. For each validator, count how many times the validator appears across all `pfReport`s in the epoch (each round change adds one entry). PFS maps each validator address to its total proposal failure count.

Format: `pfs(k) -> {addr: failureScore}`

**Candidate Failure Score (CFS)**: For epoch index `k`, CFS MUST be computed from `cfReport(N)` for blocks `N ∈ [k*EPOCH_LENGTH, (k+1)*EPOCH_LENGTH - 1]` (note that `cfReport(k*EPOCH_LENGTH)` is empty).
...
Format: `cfs(k) -> {addr: failureScore}`
