...
3. The `bid.call_gas_limit` must be less or equal to `BidTxMaxCallGasLimit`.
   1. The `BidTxMaxCallGasLimit` is the maximum gas limit for the backrun logic.
...
5. The `bid.from_signature` and `bid.auc_signature` must be valid.
...
- `auction_newPendingTransactions(fullTx: bool)`

  - This is the new API returns the new full body of pending transactions. The transactions have same format as `txpool.content`, but with a new property, `time` field.
